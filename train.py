#!/usr/bin/env python3
"""
전체 학습 파이프라인 (One-Shot)
데이터 전처리 → 그래프 구성 → 모델 학습 → Threshold 최적화
"""
...
def step_5_graph_construction():
    """Step 5: Graph Construction"""
    print("\n" + "="*80)
    print("Step 5️⃣: Multi-Relation Graph Construction (6 Relations)")
    print("="*80)
...
def step_6_model_training(model_name, config_path):
    """Step 6: Model Training"""
    print("\n" + "="*80)
    print(f"Step 6️⃣: Model Training ({model_name.upper()})")
    print("="*80)

    # Import here to avoid circular imports
    import subprocess

    cmd = f"python -m src.training.train --model {model_name} --config {config_path}"
    result = subprocess.run(cmd, shell=True)

    if result.returncode != 0:
        raise RuntimeError(f"Model training failed: {model_name}")

    print(f"\n✅ {model_name.upper()} 모델 학습 완료")
...
def step_8_final_evaluation(model_name):
    ...
    checkpoint = f"outputs/best_model_{model_name}.pt"
...
def main():
    parser = argparse.ArgumentParser(description="End-to-End GNN Training Pipeline")
    parser.add_argument(
        "--model",
        type=str,
        default="cage_rf_gnn",
    ...
    parser.add_argument(
        "--skip-graph",
        action="store_true",
        help="Skip graph construction (if already done)"
    )

    args = parser.parse_args()

    print("\n" + "="*80)
    print("🚀 GNN-based Fraud Detection - Full Training Pipeline")
    print("="*80)
    ...
    try:
        ...
        if not args.skip_preprocessing:
            ...
            print("\n⏭️ 데이터 전처리 건너뜀 (이미 완료된 상태)")

        # Step 5: Graph Construction
        if not args.skip_graph:
            print("\n[Phase 2] 그래프 구성 시작...")
            step_5_graph_construction()
            print(f"\n✅ 그래프 구성 완료 (6개 Relation)")
        else:
            print("\n⏭️ 그래프 구성 건너뜀 (이미 완료된 상태)")

        # Step 6-8: Model Training & Evaluation
        print(f"\n[Phase 3] 모델 학습 및 평가 시작 ({args.model})...")
        step_6_model_training(args.model, args.config)
        step_7_threshold_optimization()
        step_8_final_evaluation(args.model)
        ...
        print(f"📁 결과: outputs/ (best_model_{args.model}.pt, metrics_{args.model}.json)")
        print(f"📈 대시보드: streamlit run dashboard/app.py")
        print("="*80 + "\n")
