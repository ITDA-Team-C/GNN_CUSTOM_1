...
선택된 relation의 branch만 사용하여 signal dilution 해결.
상위 PR-AUC 기여도 branch만 유지해 파라미터 감소 & 과적합 방지.
"""
...
def step_5_graph_construction(config):
    ...
    print("\n" + "="*80)
    print("Step 5️⃣: Multi-Relation Graph Construction (Selected Relations)")
    print("="*80)
...
def step_6_model_training(config_path):
    ...
    print("\n" + "="*80)
    print(f"Step 6️⃣: Model Training (CAGE-RF-GNN v3: Branch Ablation)")
    print("="*80)
    ...
    if result.returncode != 0:
        raise RuntimeError("Model training failed: cage_rf_gnn v3")

    print(f"\n✅ CAGE-RF-GNN v3 모델 학습 완료")
...
def main():
    parser = argparse.ArgumentParser(description="V3: Branch Ablation Training Pipeline")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/v3_ablation.yaml",
        help="Config file path (default: v3_ablation.yaml)"
    )
    parser.add_argument(
        "--skip-preprocessing",
        action="store_true",
    ...
    print("\n" + "="*80)
    print("🚀 V3: Branch Ablation - Pruning for Signal Clarity")
    print("="*80)
    ...
    try:
        ...
        print(f"📊 모델: CAGE-RF-GNN v3 (Branch Ablation)")
