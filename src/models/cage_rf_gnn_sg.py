...
class CAGERFGNNBranch(nn.Module):
    def __init__(self, input_dim, hidden_dim=64, num_layers=2, dropout=0.3, K=2):
        super().__init__()
        self.convs = nn.ModuleList()
        self.convs.append(SGConv(input_dim, hidden_dim, K=K))
        for _ in range(num_layers - 1):
            self.lins = nn.ModuleList([nn.Linear(hidden_dim, hidden_dim) for _ in range(num_layers - 1)])
...
class CAGERF_GNN(nn.Module):
    def __init__(self, input_dim, hidden_dim=64, num_layers=2, dropout=0.3, use_gating=True,
                 use_ensemble=False, selected_relations=None, K=2):
        super().__init__()
