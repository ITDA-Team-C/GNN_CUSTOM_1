...
Gating mechanism 대신 각 relation별 독립적인 fraud classifier를 통해 개별 fraud score 생성.
Learnable ensemble weights로 relation의 기여도를 학습하여 설명가능성 향상.
"""
...
def step_6_model_training(config_path):
    ...
    print("\n" + "="*80)
    print(f"Step 6️⃣: Model Training (CAGE-RF-GNN v7: Ensemble)")
    print("="*80)
    ...
    if result.returncode != 0:
        raise RuntimeError("Model training failed: cage_rf_gnn v7")

    print(f"\n✅ CAGE-RF-GNN v7 모델 학습 완료")
...
def main():
    parser = argparse.ArgumentParser(description="V7: Ensemble Training Pipeline")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/v7_ensemble.yaml",
        help="Config file path (default: v7_ensemble.yaml)"
    )
    parser.add_argument(
        "--skip-preprocessing",
        action="store_true",
    ...
    print("\n" + "="*80)
    print("🚀 V7: Ensemble - Multi-Relation Fraud Score Fusion")
    print("="*80)
    ...
    try:
        ...
        print(f"📊 모델: CAGE-RF-GNN v7 (Ensemble)")
