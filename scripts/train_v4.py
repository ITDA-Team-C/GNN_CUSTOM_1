#!/usr/bin/env python3
"""
V4: PR-Curve Based Threshold Optimization
Valid set의 PR-Curve 위에서 F1 최대 지점의 threshold를 선택.
Macro-F1 기준보다 소수 클래스 최적화에 유리한 임계값 선택.
"""
...
def step_6_model_training(config_path):
    ...
    print("\n" + "="*80)
    print(f"Step 6️⃣: Model Training (CAGE-RF-GNN v4: PR-Curve Threshold)")
    print("="*80)
    ...
    if result.returncode != 0:
        raise RuntimeError("Model training failed: cage_rf_gnn v4")

    print(f"\n✅ CAGE-RF-GNN v4 모델 학습 완료")


def step_7_threshold_optimization():
    """Step 7: PR-Curve Based Threshold Optimization"""
    print("\n" + "="*80)
    print("Step 7️⃣: Threshold Optimization (PR-Curve 기반)")
    print("="*80)
...
def main():
    parser = argparse.ArgumentParser(description="V4: PR-Curve Based Threshold Training Pipeline")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/v4_threshold_pr.yaml",
        help="Config file path (default: v4_threshold_pr.yaml)"
    )
    parser.add_argument(
        "--skip-preprocessing",
        action="store_true",
    ...
    print("\n" + "="*80)
    print("🚀 V4: PR-Curve Based Threshold - Minority Class Optimization")
    print("="*80)
    ...
    try:
        ...
        print(f"📊 모델: CAGE-RF-GNN v4 (PR-Curve Threshold)")
