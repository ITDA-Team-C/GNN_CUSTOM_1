...
Train set에서 소수 클래스(사기 리뷰) 샘플을 반복 추출하여 불균형 문제 완화.
Recall 향상과 함께 PR-AUC & Macro-F1 동시 개선 기대.
"""
...
def step_6_model_training(config_path):
    ...
    print("\n" + "="*80)
    print(f"Step 6️⃣: Model Training (CAGE-RF-GNN v5: Oversampling)")
    print("="*80)
    ...
    if result.returncode != 0:
        raise RuntimeError("Model training failed: cage_rf_gnn v5")

    print(f"\n✅ CAGE-RF-GNN v5 모델 학습 완료")
...
def main():
    parser = argparse.ArgumentParser(description="V5: Oversampling Training Pipeline")
    parser.add_argument(
        "--config",
        type=str,
        default="configs/v5_oversampling.yaml",
        help="Config file path (default: v5_oversampling.yaml)"
    )
    parser.add_argument(
        "--skip-preprocessing",
        action="store_true",
    ...
    print("\n" + "="*80)
    print("🚀 V5: Oversampling - Minority Class Amplification")
    print("="*80)
    ...
    try:
        ...
        print(f"📊 모델: CAGE-RF-GNN v5 (Oversampling)")
