...
def load_data():
    ...
    print(f"  Available edge keys: {list(edge_data.keys())}")

    expected_keys = ['rur', 'rtr', 'rsr']
    edge_indices = []
    for key in expected_keys:
        if key in edge_data:
            ...
            raise KeyError(f"Edge key '{key}' not found. Available keys: {list(edge_data.keys())}. "
                          f"Re-run preprocess.py to regenerate edge_index_dict.pt")
...
def main():
    ...
    with open(f"outputs/output_yelpchi/results/results_{timestamp}.json", 'w') as f:
